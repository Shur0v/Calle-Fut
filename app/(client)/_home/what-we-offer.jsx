...
export default function WhatweOffer() {
  return (
    <div className="w-full bg-[#f8f8f8] relative">
      ...
      <div className="container mx-auto max-w-[1324px] py-[50px] md:py-[100px] px-4 md:px-0">
        <h1 className='text-3xl md:text-5xl font-bold text-center text-[#070707] mb-8 md:mb-14'>
          Elevate Your Game with Expert Training!
        </h1>

        <div className="grid grid-cols-1 md:grid-cols-2 gap-5 md:gap-7 justify-center">
          {trainingData.map((card, index) => (
            <div key={index} className="w-full md:w-[648px] h-auto p-5 md:p-10 bg-white rounded-lg flex-col justify-start items-start gap-4 md:gap-6 inline-flex">
              <div className="self-stretch text-[#070707] text-xl md:text-2xl font-semibold leading-normal">{card.title}</div>
              <div className="self-stretch flex-col justify-start items-start gap-4 flex">
                <div className="flex-col justify-start items-start gap-[18px] flex">
                  <div className="self-stretch text-[#4a4c56] text-lg md:text-xl font-medium leading-tight">{card.subtitle}</div>
                      ...
                      <div key={idx} className="justify-start items-center gap-3 inline-flex">
                        ...
                        <div className="text-center">
                          {typeof feature === 'string' ? (
                            <span className="text-[#4a4c56] text-sm md:text-base font-normal leading-tight">{feature}</span>
                          ) : (
                            <>
                              <span className="text-[#4a4c56] text-sm md:text-base font-semibold leading-tight">{feature.text}</span>
                              <span className="text-[#4a4c56] text-sm md:text-base font-normal leading-tight">{feature.highlight}</span>
                            </>
                          )}
                        </div>
                      </div>
                ...
                <div className="self-stretch px-5 md:px-10 py-6 md:py-8 bg-[#f7f7f7] rounded-xl flex-col justify-center items-start gap-4 md:gap-5 flex mt-4">
                  <div className="text-[#070707] text-xl md:text-2xl font-semibold leading-normal">Packages</div>
                  <div className="self-stretch flex-col justify-center items-start gap-3 md:gap-4 flex">
                    {card.packages.map((pkg, idx) => (
                      <div key={idx} className="self-stretch">
                        <span className="text-[#1d1f2c] text-lg md:text-xl font-normal leading-tight">{pkg.sessions} Sessions:</span>
                        <span className="text-[#1d1f2c] text-lg md:text-xl font-medium leading-tight"> </span>
                        <span className="text-[#b60000] text-lg md:text-xl font-medium leading-tight">${pkg.price}</span>
                      </div>
                    ))}
                  </div>
                </div>
